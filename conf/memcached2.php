<?php
    ...
    // Run the Memcached checks first, collecting results so we can render them
    ...
    if (!class_exists('Memcached')) {
        $checks[] = array(
            'tag'    => 'Extension',
            'title'  => 'PHP Memcached extension',
            'ok'     => false,
            'detail' => 'The PHP Memcached extension was not installed.',
        );
    } else {
        $checks[] = array(
            'tag'    => 'Extension',
            'title'  => 'PHP Memcached extension',
            'ok'     => true,
            'detail' => 'The PHP Memcached extension is loaded.',
        );

        $mem = new Memcached();
        $mem->addServer("127.0.0.1", 11211);

        // addServer() only registers the server; getVersion() actually probes it.
        $versions = $mem->getVersion();
        $version = is_array($versions) ? current($versions) : false;

        if ($version === false || $version === '' || $version === '255.255.255') {
            $checks[] = array(
                'tag'    => 'Connection',
...
<head>
  ...
  <title>GetLNMP Memcached Test</title>
  <meta name="author" content="GetLNMP">
  <meta name="keywords" content="GetLNMP,LNMP,LNMPA,LAMP,Nginx,MySQL,MariaDB,PHP,Memcached">
  <meta name="description" content="GetLNMP Memcached connection test tool.">
  <style>
    :root {
      color-scheme: light;
      --bg: #f5f7f8;
...
<body>
  <main class="dashboard">
    <header class="topbar">
      <span class="brand">
        ...
        <span>
          ...
          <span class="brand-sub">Memcached Test</span>
        </span>
      </span>
    ...
    <section class="hero" aria-labelledby="memcached-title">
      <h1 id="memcached-title">Memcached connection test</h1>
      <p>
        <?php if ($allOk): ?>
          The PHP Memcached extension is installed and the Memcached server at
        ...
        <?php else: ?>
          ...
          Memcached extension or server connection.
        <?php endif; ?>
      </p>
    </section>

    <section class="grid" aria-label="Memcached test results">
