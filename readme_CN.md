...
支持自定义Nginx、PHP编译参数及网站和数据库目录、支持生成Let's Encrypt/ZeroSSL免费SSL证书、支持无人值守、LNMP模式支持多PHP版本、支持单独安装Nginx/MySQL/MariaDB/PureFTPd服务器，同时提供一些实用的辅助工具如：虚拟主机管理、防火墙管理、FTP用户管理、Nginx、MySQL/MariaDB、PHP的升级、常见PHP模块exif、fileinfo、ldap、bz2、sodium、imap和swoole的一键安装、常用缓存组件Redis、Memcached、OPcache和APCu的安装、重置MySQL root密码、502自动重启、日志切割、SSH防护Fail2Ban、备份等许多实用脚本。
...
### 防火墙管理
`conf/lnmp-fw` 是通用端口防火墙管理脚本，RHEL/Rocky/Alma/CentOS/Fedora 系统使用 `firewall-cmd`，Debian/Ubuntu 系统使用 `ufw`。该脚本的防火墙后端支持范围独立于 GetLNMP 主安装脚本的系统支持范围。

使用前可将其复制到系统命令路径，例如：`cp conf/lnmp-fw /usr/local/bin/lnmp-fw`。

常用命令：
* 允许端口：`lnmp-fw allow <port> [tcp|udp|all]`
* 拒绝端口：`lnmp-fw deny <port> [tcp|udp|all]`
* 删除允许规则：`lnmp-fw delete-allow <port> [tcp|udp|all]`
* 删除拒绝规则：`lnmp-fw delete-deny <port> [tcp|udp|all]`
* 查看防火墙状态：`lnmp-fw status`
* 重新加载防火墙：`lnmp-fw reload`

示例：
* 允许 HTTP：`lnmp-fw allow 80 tcp`
* 允许 HTTPS：`lnmp-fw allow 443 tcp`
* 允许 DNS UDP：`lnmp-fw allow 53 udp`
* 同时允许 TCP/UDP：`lnmp-fw allow 53 all`
* 拒绝 MySQL 端口：`lnmp-fw deny 3306 tcp`
* 删除 8080 端口允许规则：`lnmp-fw delete-allow 8080 tcp`

注意：对 SSH 端口执行 `deny` 或 `delete-allow` 时会要求交互确认；无人值守或脚本中执行需添加 `--force`，如：`lnmp-fw deny 22 tcp --force`，或设置 `LNMP_FW_FORCE=1`。
...
**无人值守命令生成工具：<https://getlnmp.com/auto.html>**
* 设置如下环境变量即可完全无人值守安装
...
Bin | (非必选)MySQL 5.7/8.0/8.4和MariaDB使用二进制方式安装，y 或 n，默认使用二进制方式安装，离线默认使用源码编译安装

Apache始终以源码方式安装2.4版本，不需要Apache版本变量。

* 各程序版本对应序号

数据库(`DBSelect`) | 对应序号 | PHP(`PHPSelect`) | 对应序号 | 内存分配器(`SelectMalloc`) | 对应序号
:------: | :------: | :------: | :------: | :------: | :--------:
MySQL 5.7 | 3 | PHP 7.2 | 8 | 不安装(默认) | 1
...
MariaDB 11.4(默认) | 11 | PHP 8.4 | 15 | |
MariaDB 11.8 | 12 | PHP 8.5 | 16 | |
不安装数据库 | 0 | | | |

交互式安装默认使用MariaDB 11.4 (`DBSelect=11`) 和 PHP 8.3 (`PHPSelect=14`)。

* 以LNMP模式安装MariaDB 11.4、数据库root密码设置为getlnmp.com、启用InnoDB、PHP 8.3、不安装内存分配器为例，先执行([建议先运行screen](https://www.vpser.net/manage/run-screen-lnmp.html))，再下载解压LNMP安装包：
...
| 参数名称 | 参数介绍 | 示例/默认值 |
| :-------: | :---------: | :--------: | 
|Download_Mirror|下载镜像，`Use_Official=n` 时使用|默认：`https://lax.getlnmp.com`，下载异常时可修改|
|Use_Official|从官方上游站点下载文件|默认：`y`；设为 `n` 时使用 `Download_Mirror`|
|country|服务器国家代码，用于选择更近的下载镜像|例如：`US`|
|Nginx_Modules_Options|添加Nginx模块或其他编译参数|`--add-module=/第三方模块源码目录`|
|PHP_Modules_Options|添加PHP模块或编译参数|`--enable-exif`，有些模块需提前安装好依赖包|
|OS_Timezone|系统时区|默认：`Etc/UTC`|
|PHP_Timezone|PHP时区|默认：`UTC`|
|Open_DB_Port|是否在防火墙中开放3306端口以允许远程数据库访问|默认：`n`|
|MySQL_Data_Dir|MySQL数据库目录设置|默认：`/usr/local/mysql/data`|
|MariaDB_Data_Dir|MariaDB数据库目录设置|默认：`/usr/local/mariadb/data`|
|Default_Website_Dir|默认虚拟主机网站目录位置|默认：`/home/wwwroot/default`|
|Enable_Nginx_Lua|是否为Nginx安装Lua支持|默认：`n`，Lua支持可用于一些基于Lua的WAF网站防火墙|
|Enable_Ngx_FancyIndex|是否安装fancyIndex模块|默认：`n`，fancyIndex是一个第三方目录索引模块|
|Enable_Swap|是否添加SWAP|默认：`y`，当内存不足时可提高编译安装成功概率|
|Enable_PHP_Exif|是否添加PHP exif模块|默认：`n`，更改为 `y` 即安装|
|Enable_PHP_Fileinfo|是否添加PHP fileinfo模块|默认：`y`，需1GB以上内存|
|Enable_PHP_Ldap|是否添加PHP ldap模块|默认：`n`，更改为 `y` 即安装|
|Enable_PHP_Bz2|是否添加PHP bz2模块|默认：`n`，更改为 `y` 即安装|
|Enable_PHP_Sodium|是否添加PHP sodium模块，PHP 7.2以下不支持通过`lnmp.conf`开启安装|默认：`y`|
|Enable_PHP_Imap|是否添加PHP imap模块|默认：`n`，更改为 `y` 即安装|
|SelectMalloc|MySQL/MariaDB升级时继续使用的内存分配器（`1` 不安装，`2` Jemalloc，`3` TCMalloc）|设置为安装时选择的值|
